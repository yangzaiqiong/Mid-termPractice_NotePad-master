...
public class NoteEditor extends Activity{
    ...
    protected void onResume() {
        ...
        if (mCursor != null) {
            ...
            //设置背景
            int backgroundIndex = mCursor.getColumnIndex(NotePad.Notes.COLUMN_NAME_BACKGROUND);
            String colorText = mCursor.getString(backgroundIndex);
            mText.setBackgroundColor(Color.parseColor(colorText));

            //设置字体颜色
            int textColorIndex = mCursor.getColumnIndex(NotePad.Notes.COLUMN_NAME_TEXT_COLOR);
            String colorText2 = mCursor.getString(textColorIndex);
            mText.setTextColor(Color.parseColor(colorText2));

            //设置字体颜色
            int textSizeIndex = mCursor.getColumnIndex(NotePad.Notes.COLUMN_NAME_TEXT_SIZE);
            int size = mCursor.getInt(textSizeIndex);
